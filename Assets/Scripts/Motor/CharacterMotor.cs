...
public class CharacterMotor : RaycastMotor, IMotor {
			...
			if (hit) {
				...
				if(!collision.slopeUp || slopeAngle > maxSlopeAngle) {
					...
					if (collision.slopeUp) {
						move.y = Mathf.Tan(collision.slopeAngle * Mathf.Deg2Rad) * Mathf.Abs(move.x);
					}
	...
	void VerticalCollisions(ref Vector2 move) {
		...
		for (int i = 0; i < verticalRayCount; i++) {
			...
			if (hit) {
				...
				if (collision.slopeUp) {
					move.x = move.y / Mathf.Tan(collision.slopeAngle * Mathf.Deg2Rad) * Mathf.Sign(move.x);
				}
	...
	void ClimbingSlope(ref Vector2 move, float slopeAngle){
		...
		//note: if y is higher, that the player is jumping (or otherwise changing the y move)
